...
# Update the observation window buffer (61 frames: 60 past + 1 current, aligned with model.py)
def update_observation_window(args, config, ros_operator):
    ...
    img_size = tuple(getattr(args, 'img_size', [640, 480]))

    global observation_window

    img_front, img_left, img_right, puppet_arm_left, puppet_arm_right = (get_ros_observation(args, ros_operator))
    # Legacy compatibility: keep the same numeric channel order as model.py.
    # RoboTwin sim images are numeric RGB, and historical HDF5 training data was
    # encoded with cv2.imencode directly on numeric RGB arrays. This JPEG
    # round-trip preserves that numeric RGB order for SigLIP consistency.
    # WAN VAE online encoding still applies the legacy channel flip in
    # agilex_model.py to match existing precached latents.
    img_front = jpeg_mapping(resize_img(img_front, img_size))
    img_left  = jpeg_mapping(resize_img(img_left,  img_size))
    img_right = jpeg_mapping(resize_img(img_right, img_size))
    ...
    if observation_window is None:
        # Fix 1+6: maxlen=61, init with 61 copies of first real obs (not None dummy)
        observation_window = deque(maxlen=61)
        for _ in range(61):
            observation_window.append(current_obs)
    else:
        observation_window.append(current_obs)


# Video2Act inference — aligned with inference_fn_with_history in model.py
def inference_fn(args, config, policy, t,
                 text_instruction=None,
                 wan_text_embed_path=None):
    global observation_window
    global lang_embeddings

    while True and not rospy.is_shutdown():
        # Fix 2: dense images — (t-1, t) × 3 cameras, same as training
        dense_image_arrs = [
        ...
        images = [PImage.fromarray(arr) if arr is not None else None for arr in dense_image_arrs]

        # Fix 2: motion stream needs all 61 consecutive frames (not sparse-sampled).
        # Runner expects latent T = (61-1)//4+1 = 16 and will raise ValueError otherwise.
        sparse_image_arrs = [
            observation_window[i]["images"][cam]
            for i in range(len(observation_window))  # all 61 consecutive frames
            for cam in config["camera_names"]
        ]
        sparse_images = [PImage.fromarray(arr) if arr is not None else None for arr in sparse_image_arrs]

        # get last qpos [14] → unsqueeze to [1, 14]
        proprio = observation_window[-1]["qpos"].unsqueeze(0)

        # Fix 2+3+4+7: pass sparse_images, wan_text_embeds, text_instruction, is_new_task
        actions = (
            policy.step(
                proprio=proprio,
                images=images,
                ...
                is_new_task=True,
                wan_text_embeds=wan_text_embed_path,
            )
            .squeeze(0).cpu().numpy()
        )
        return actions
...
def model_inference(args, config, ros_operator):
    ...
    text_instruction = lang_dict.get("instruction", "")

    wan_text_embed_path = getattr(args, "wan_text_embed_path", None)
            ...
            while t < max_publish_step and not rospy.is_shutdown():
                ...
                if t % chunk_size == 0:
                    ...
                    action_buffer = inference_fn(
                        args, config, policy, t,
                        text_instruction=text_instruction,
                        wan_text_embed_path=wan_text_embed_path,
                    ).copy()
...
class RosOperator:
    ...
    def puppet_arm_publish_linear(self, left, right):
        ...
        for i in range(len(traj_left_list)):
            ...
            joint_state_msg = JointState()
            joint_state_msg.header = Header()
            joint_state_msg.header.stamp = rospy.Time.now()  # timestamp
            joint_state_msg.name = [
                "joint0",
                "joint1",
                "joint2",
...
def get_arguments():
    ...
    parser.add_argument(
        "--config_path",
        type=str,
        default="configs/video2act_template.yaml",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to pre-computed WAN UMT5 text embeddings (.pkl) for WAN world model conditioning",
    )
    parser.add_argument(
        "--img_size",
        type=int,
        ...
        default=[640, 480],
        metavar=("W", "H"),
        help="Image resize target (width height) before JPEG encode, e.g. --img_size 640 480",
    )
